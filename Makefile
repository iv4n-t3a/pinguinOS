...
build/mbr.img: build/mbr.o boot/mbr.ld
	mkdir -p build
	$(LD) $(LD_FLAGS) -T boot/mbr.ld -o build/mbr.img build/mbr.o

build/mbr.o: boot/mbr.S
	$(AS) $(AS_FLAGS) -o build/mbr.o boot/mbr.S

build/vbr.img: build/vbr.o boot/vbr.ld
	mkdir -p build
	$(LD) $(LD_FLAGS) -T boot/vbr.ld -o build/vbr.img build/vbr.o

build/vbr.o: boot/vbr.S
	$(AS) $(AS_FLAGS) -o build/vbr.o boot/vbr.S

build/kernel.img: build/kernel.o kernel/kernel.ld
	mkdir -p build
	$(LD) $(LD_FLAGS) -T kernel/kernel.ld -o build/kernel.img build/kernel.o

build/kernel.o: kernel/kernel.S
	$(AS) $(AS_FLAGS) -o build/kernel.o kernel/kernel.S
